...
class HumanoidArmLiftVideoEnv:
    ...
    def load_robot_asset(self):
        ...
        self.robot_asset = self.gym.load_asset(
            self.sim, 
            "/home/ishaan_essential_ai/ucr/ucr-humanup/simulation/legged_gym/resources/robots/g1_modified", 
    ...
    def update_arm_animation(self, step):
        ...
        arm_targets = {
            'right_shoulder_pitch_joint': -0.8,  # Move arm forward (negative for right arm)
            'right_shoulder_roll_joint': -1.2,   # Move arm away from body (negative for right arm)
            'right_shoulder_yaw_joint': 0.5,     # Slight rotation
            'right_elbow_joint': 1.5              # Bend elbow significantly
        }
...
def main():
    ...
    video_path = os.path.join(video_dir, "humanoid_arm_lift_fixed.mp4")
